...
export default function ProjectSessionDetailPage({
  ...
  }, [activeTab, sessionCompleted, contentPodReady, contentPodSpawning]);
  ...
  const spawnContentPodAsync = async () => {
    ...
    try {
      ...
      const pollInterval = setInterval(async () => {
        ...
        try {
          ...
          if (attempts >= maxAttempts) {
            clearInterval(pollInterval);
            setContentPodSpawning(false);
            errorToast('Workspace viewer failed to start');
          }
        } catch {
      ...
      setContentPodSpawning(false);
      errorToast(error instanceof Error ? error.message : 'Failed to spawn workspace viewer');
    }
  };
